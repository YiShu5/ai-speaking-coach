...
Seven independent DeepSeek calls (`deepseek-chat`, `response_format: json_object`), orchestrated in three stages in `src/app/api/report/generate/route.ts`: five dimension coaches run in parallel (each with only its own persona + rubric, blind to the others), then the optimizer coach receives their five diagnoses and writes the optimized script, then the overall reviewer synthesizes all six conclusions. Each agent call retries once on failure; a second failure falls through to the fallback report. Splitting the calls made generation *faster* than the old single mega-call (~10s vs 30-60s) because stage 1 is parallel and each response is small — don't "optimize" it back into one call.
